...
class Menu extends Component {
  ...
  handleQueryChange = e => {
    ...
    this.setState({
      query,
      filteredCities: cities
        .filter(city => city.toLowerCase().indexOf(query.toLowerCase()) !== -1)
        .sort()
    });
  };

  render() {
    const { query, filteredCities, rowHeight } = this.state;
      ...
      <div className={cc(styles.menu, "menu")}>
        ...
        {rowHeight && (
          <VirtualList
            ...
            data={filteredCities}
            renderRow={row => (
              <div className={styles.menuCitiesItem} onClick={() => this.handleCityClick(row)}>
                {row}
              </div>
            )}
            rowHeight={40}
  ...
  updateRowHeight = () => {
    ...
    const canonicalWidth = width <= 600 ? 414 : width <= 1024 ? 1024 : 1920;
