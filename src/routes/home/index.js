...
import { XS, S, M, L, XL, XXL } from "../../components/text";
import { tempToStr, cc, withClass, easeInOutQuad } from "../../utils";
...
const getWeatherByQuery = async weatherQuery => {
...
const getWeather = async givenLocation => {
  const query = {};
  if (givenLocation) {
    query["location"] = givenLocation;
  } else {
    try {
      const position = await getLocationFromDevice();
  ...
  if (isDefault) {
    const defaultWeather = await getWeatherByQuery({ location: "Paris, France" });
    return {
      weather: {
        ...defaultWeather,
        city: "Paris"
    ...
    return {
      weather: {
        ...weather,
        city: query.location ? query.location.split(",")[0] : weather.location.city
      }
    };
  }
};
...
const Announcement = ({ description, sunriseHours, sunsetHours, isNight }) => (
  <Column>
...
const ForecastItem = ({ label, value, className }) => (
...
const Forecast = ({ weather, isFahrenheitOn, currentTime }) => (
  <Column>
    ...
    <ColumnContent>
      <ForecastItem label="Time" value={currentTime} className={styles.forecastItemTime} isFirst />
      <ForecastItem
        label="Sunrise"
      ...
      />
      <ForecastItem label="Wind" value={weather.current_observation.wind.speed + "m/s"} />
    </ColumnContent>
  </Column>
);
...
const getTheme = (weatherConditionCode, isDefault, isNight) => {
  return isNight
    ? NIGHT
    : isDefault || !THEMES[weatherConditionCode] || typeof THEMES[weatherConditionCode] === "string"
...
const getDescription = (weather, isFahrenheitOn) => {
  ...
  return [
    ...
    `It will ${
      currentCode === todayCode ? "stay" : "evolve to"
    } ${todayText}, with a <b>maximum temperature of ${todayHigh}</b>`,
...
const Credits = () => (
  <XS as="div" className={styles.credits}>
    ...
    <span>
      Design:{" "}
      <a href="https://ilyanaumoff.com" target="_blank" rel="noopener noreferrer">
        Ilya Naumoff
      </a>
    </span>
    <span>
      Development:{" "}
      <a href="https://sergeyzakharov.dev" target="_blank" rel="noopener noreferrer">
        Sergey Zakharov
      </a>{" "}
      and{" "}
      <a href="https://black-foundry.com" target="_blank" rel="noopener noreferrer">
        Black[Foundry]
      </a>{" "}
    </span>
...
class Home extends Component {
  ...
  render() {
    ...
    if (isLoading) {
      ...
      const { className, title, icon, audio } = theme;
      return (
        <HomeBox className={className}>
          ...
          />
          <div className={cc(styles.slogan, "slogan")}>{title}</div>
  ...
  startLoadingWeather = location => {
    ...
    return Promise.all([getWeather(location), loadVariableFont()]).then(
      ([{ weather, isDefault }]) => {
        this.setState(
          {
            weather,
